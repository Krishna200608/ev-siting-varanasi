...
> 🚀 **Live Interactive Web Application:** [https://ev-siting-varanasi.streamlit.app](https://ev-siting-varanasi.streamlit.app)  
...
- [🗂️ Directory Structure & Synopsis Mapping](#-directory-structure--synopsis-mapping)
- [⚙️ Environment Setup & Installation](#-environment-setup--installation)
...
| 🛡️ **Methodological Stability** | **$\rho \ge 0.88$** | Spearman rank correlation exceeds 0.88 across 11 of 12 perturbation scenarios; uncovered non-linear scale dependence in arterial road proximity ($S_{11}$: sample $\rho = 0.9613 \to$ citywide $\rho = 0.7763$). |
...
flowchart TD
    subgraph Stage1["📍 Stage 1: Spatial Suitability (Where to Site)"]
        ...
        D1 --> E1[Objective Weighting: CRITIC & Entropy]
        E1 --> F1[MCDM Ranking: TOPSIS & WASPAS]
    end

    subgraph Stage2["⏰ Stage 2: Operational Demand Profiling (When Demand Occurs)"]
        G1[Caltech/JPL ACN-Data<br>30,000+ Real Transactions] --> H1[Feature Engineering & Temporal Encoding]
        H1 --> I1[XGBoost 5-Fold Cross-Validation]
        I1 --> J1[SHAP Feature Attribution]
        J1 --> K1[24-Hour Diurnal Demand Load Curve<br>Weekday vs. Weekend]
    end
    ...
    style Stage1 fill:#f0fdf4,stroke:#16a34a,stroke-width:2px
    style Stage2 fill:#eff6ff,stroke:#2563eb,stroke-width:2px
    style Stage3 fill:#faf5ff,stroke:#9333ea,stroke-width:2px
```
...
## 🗂️ Directory Structure & Synopsis Mapping
...
### ⚖️ Stage 2: MCDM Weighting & Siting Optimization
...
| **Page 1** | `:material/map: 1_Site_Map.py` | Full-screen interactive Folium map displaying all 308 candidate alternatives colored by TOPSIS suitability with Top-5 gold-bordered markers, click popups, and a v1 vs. v2 spatial scrutiny comparison toggle. |
| **Page 2** | `:material/leaderboard: 2_MCDM_Rankings.py` | Searchable, sortable consolidated rankings table across all 4 MCDM combinations with urban zone filters (Godowlia, Sigra, Lanka, Cantt, Peri-Urban), Spearman rank concordance matrices, and CSV export. |
| **Page 3** | `:material/tune: 3_Whatif_Weight_Explorer.py` | Real-time interactive sensitivity tool with 9 criteria sliders and quick presets (CRITIC default, Equal weights, Road arterial focus, Mall focus, Hospital focus) computing live TOPSIS re-rankings and rank-shift scatter plots. |
| **Page 4** | `:material/show_chart: 4_Demand_and_SHAP.py` | 24-hour diurnal energy load curves (weekday midday peak vs. weekend morning charge), SHAP feature importance plots, and methodological synthesis on operational timing (RQ3). |
| **Page 5** | `:material/query_stats: 5_Sensitivity_Analysis.py` | 12-scenario criteria weight perturbation analysis, radar/bar plots, and scale-dependent road proximity dynamics ($S_{11}$: $\rho = 0.9613 \to 0.7763$). |
| **Page 6** | `:material/verified_user: 6_Data_Quality_Audit.py` | Systematic 9-criteria $\times$ 2-version audit table, automated degeneracy safeguards, and root-cause diagnostics on $C_5$ (Competitor EVCS) and $C_6$ (Hospitals). |
| **Page 7** | `:material/menu_book: 7_Project_Journey.py` | Grounded chronological record of architectural decisions (AD-1 through AD-11) documenting problems encountered, empirical findings, and verified solutions. |
...
**Krishna Sikheriya**  
*Managing Corporate Entrepreneurship Research Project*  
