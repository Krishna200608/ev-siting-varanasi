...
def generate_mcdm_sensitivity_figure(
    ...
    grid_color = "#30363D" if is_dark else "#E2E8F0"
    border_color = "#30363D" if is_dark else "#CBD5E1"
    color1 = "#38BDF8" if is_dark else "#1f77b4"
    color2 = "#4ADE80" if is_dark else "#2ca02c"
    ...
    bars1 = ax1.bar(x - width / 2, sensitivity_df["spearman_rho"], width, label="Spearman Rank Correlation (ρ)", color=color1, alpha=0.9)
    ax1.set_xlabel("Sensitivity Scenario (Rashmitha et al., 2024 Framework)", fontsize=11, labelpad=10, color=text_color)
    ...
    ax2 = ax1.twinx()
    bars2 = ax2.bar(x + width / 2, sensitivity_df["top5_overlap_pct"], width, label="Top-5 Shortlist Overlap (%)", color=color2, alpha=0.9)
    ...
    ax1.set_title("MCDM Criteria Weight Sensitivity Analysis (TOPSIS-CRITIC Robustness)", fontsize=13, pad=15, fontweight="bold", color=text_color)
